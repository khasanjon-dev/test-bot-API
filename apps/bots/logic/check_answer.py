...
from bots.data import data
from bots.logic.menu import main_menu_handler
from bots.utils import states, request
from bots.utils.keyboardbuilder import keyboard_builder
from bots.utils.serializers import keys_serializer
from root.settings import ADMIN_USERNAME
...
async def block_result(msg: Message, state: FSMContext, get_data: dict):
    ...
    if status == 201:
        ...
        await msg.answer(f"⚠️ Xatolik aniqlandi\n"
                         f"Iltimos adminga murojat qiling!\n"
                         f"✍️ {ADMIN_USERNAME}", reply_markup=markup)
        await state.clear()
        await state.set_state(states.Menu.main_menu)
